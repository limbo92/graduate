function [AZ,best_hgroute,best_hgvalue,hg_av,hg_mo,hg_index]=hg3(z1,z2,PT,GT,RA,RD,GO,GC,pn,gate,Z8,interation,preassigned,ro,price,people,in)
%无遗传较优解转化的信息素，只有蚁群算法
...
hg_av=zeros(1,interation);%蚁群算法每次迭代得到的最优解的干扰值
hg_mo=zeros(1,interation);%蚁群算法每次迭代得到的最优解的损失值
best_nroute=cell(1,interation-in);%存储每次迭代最优解的路径
best_nvalue=zeros(1,interation-in);%存储每次迭代的最优解的值
interval=8;%航班之间的安全间隔
initial_Tau=max_Tau/2+Z8;%蚁群初始时信息素浓度 信息素浓度最大值的一半+遗传算法Z8得到的路径信息素
while NC<=interation-in  %蚁群算法迭代interation-in次,in是遗传迭代次数
    ...
    hg_av(1,NC)=ambition(1,min_aindex);%每次迭代最小的干扰值
    hg_mo(1,NC)=ambition(2,min_aindex);%每次迭代最小的损失值
...
best_hgroute=best_nroute{1,min_nindex};%最优的再分配方案
best_hgvalue=min_nroute;%最小的目标值
hg_index=min_nindex;%最优值出现的迭代次数