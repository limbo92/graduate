...
[NZ1,a2,d2,delaytime1]=crossover1(PC,Z,NCmax,z1,NZ1,RA,RD,interval);%交叉函数，NZ1为交叉后新染色体存放的新种群
Z3=zeros(1,NCmax);%已经变异的染色体，需要排除掉，以免再次选到
%global p%
%p=1;
NZ2=zeros(1,z1);%NZ2用来存储所有变异操作出来的新染色体
[NZ2,c1,delaytime2]=variation1(NCmax,PV,Z,z1,NZ2,RA,RD,interval);%变异函数
...
delaytime4=[delaytime(b(1:NCmax*0.1),:);delaytime1;delaytime2];% NZ3染色体组的延迟时间
[NZ4,k]=evolution(best_route,NZ3,z1,preassigned,NCmax,delaytime4,people,PT,price);%该函数把初始解的最好的前10%best_route，和新交叉，变异出来的染色体放到一个新的数组里NZ3，重新计算一下，查看是否得到了进化
...
[AZ,best_acroute,best_acvalue]=aco(z1,z2,PT,GT,RA,RD,GO,GC,pn,gate,Z8,interation,preassigned);%蚁群算法
toc