function [AZ,best_acroute,best_acvalue,aco_av,aco_mo,best_nvalue]=aco1(z1,z2,PT,GT,RA,RD,GO,GC,pn,gate,Z8,interation,preassigned,ro,price,people)
...
aco_mo=zeros(1,interation);%蚁群算法每次迭代得到的最优解的多目标值
...
while NC<=interation  %蚁群算法迭代interation次
    ...
    ambition1(2,:)=ambition(2,:)/max(ambition(2,:));%多目标值规范化
    ...
    aco_av(1,NC)=ambition(1,min_aindex);%
    aco_mo(1,NC)=ambition(2,min_aindex);%
